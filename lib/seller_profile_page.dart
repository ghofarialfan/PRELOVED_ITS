...
class SellerProfilePage extends StatefulWidget {
  /// ownerId = auth.users.id (seller)
  final String ownerId;

  const SellerProfilePage({super.key, required this.ownerId});
...
class _SellerProfilePageState extends State<SellerProfilePage> {
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      final storeRes = await _client
          .from('stores')
          .select('id, owner_id, name, username, bio, location, photo_url')
          .eq('owner_id', widget.ownerId)
          .maybeSingle();

      if (storeRes == null) {
        throw Exception(
          'Store untuk owner_id ${widget.ownerId} belum ada. Insert dulu ke tabel stores.',
        );
      }

      // 2) products + join category name
      final prodRes = await _client
          .from('products')
          .select(
            'id, name, price, discount_price, created_at, seller_id, category_id, category:categories(id,name)',
          )
          .eq('seller_id', widget.ownerId)
      ...
      // 4) categories dari produk seller (distinct + count)
      ...
      // 5) reviews seller (ambil dari semua product_ids seller)
      double avg = 0;
      int count = 0;
      final List<Map<String, dynamic>> reviews = [];

      if (productIds.isNotEmpty) {
        // ambil review + join users untuk avatar/nama (kalau FK ada)
        final revRes = await _client
            .from('product_reviews')
            .select(
              'id, product_id, rating, comment, created_at, image_url, user:users(id, full_name, username, profile_image_url, photo_url)',
            )
            .inFilter('product_id', productIds)
            .order('created_at', ascending: false);
      ...
      setState(() {
        _store = Map<String, dynamic>.from(storeRes);
        _products = products;
  ...
  Widget build(BuildContext context) {
    ...
    final store = _store!;
    final storeName = (store['name'] ?? 'Toko').toString();
    final username = (store['username'] ?? 'Penjual').toString();
    final location = (store['location'] ?? '').toString();
  ...
  // ============================
  // TAB: KATEGORI (list seperti figma)
  // ============================
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        if (user is Map) {
          ...
          // beberapa tabel user kamu punya profile_image_url / photo_url
